  ...
  return (
    <nav className={`fixed top-0 w-full z-[100] transition-all duration-300 bg-[#050505] border-b border-white/10 shadow-lg shadow-black/50`}>
      <div className="container-fluid h-16 md:h-20 flex items-center justify-between">
        ...
        {/* Desktop Links */}
        <div className="hidden lg:flex items-center gap-2 xl:gap-6 ml-auto md:ml-20 xl:ml-32 justify-end">
          {navLinks.map((link, i) => (
            <motion.a key={link.href} href={link.href}
              initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * i, duration: 0.3 }}
              className={`px-1.5 py-2 rounded-md text-[0.7rem] xl:text-[0.8rem] font-bold tracking-[0.15em] uppercase transition-all duration-300 ${
          ...
          <motion.a href="#contact" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}
            className="ml-1 xl:ml-4 btn-red !py-2 !px-4 !text-[0.65rem] xl:!text-[0.7rem]">
            Hire Me
          </motion.a>
        </div>
