          ...
          <AnimatePresence>
            {displayedCerts.map((cert, index) => (
              <motion.div 
                ...
                className="w-40 h-28 sm:w-48 sm:h-32 md:w-56 md:h-36 rounded-xl border border-[#E11D48]/30 bg-[#0a0a0a] shadow-[0_0_15px_rgba(225,29,72,0.1)] hover:shadow-[0_0_30px_rgba(225,29,72,0.3)] hover:border-[#E11D48] flex flex-col items-center justify-center p-4 transition-all duration-300 group relative"
              >
                ...
                <span className="text-slate-100 font-bold text-xs sm:text-sm text-center leading-tight mb-3 sm:mb-4 relative z-10 line-clamp-3">
                  {cert.title}
                </span>
                
                <button 
                  onClick={() => setSelectedImg(cert)}
                  className="px-4 py-1.5 text-[0.65rem] sm:text-xs font-mono uppercase tracking-widest border border-[#E11D48] text-[#E11D48] rounded-full hover:bg-[#E11D48] hover:text-white transition-all duration-300 relative z-10"
                >
                  View
                </button>
              </motion.div>
      ...
      <AnimatePresence>
        {selectedImg && (
          <motion.div 
            ...
            className="fixed inset-0 z-[1000] bg-[#050505]/95 backdrop-blur-xl flex items-center justify-center p-4 md:p-12"
            onClick={() => setSelectedImg(null)}>
            
            <button 
              className="absolute top-6 right-6 md:top-8 md:right-8 bg-white/10 hover:bg-[#E11D48] text-white p-3 rounded-full transition-colors duration-300 backdrop-blur-md z-50" 
              onClick={() => setSelectedImg(null)}>
              <FiX size={24} />
            </button>
            ...
            <motion.div 
              ...
              <div className="w-full p-2 bg-[#0a0a0a] border border-[#E11D48]/50 rounded-2xl shadow-[0_0_80px_rgba(225,29,72,0.15)] relative overflow-hidden">
                {/* Modal scanline */}
                ...
                <img 
                  ...
                  className="w-full max-h-[80vh] object-contain rounded-xl relative z-0" 
                />
              </div>
              
              <div className="mt-6 flex items-center gap-3 bg-[#E11D48]/10 border border-[#E11D48]/30 backdrop-blur-md px-6 py-3 rounded-full shadow-[0_0_20px_rgba(225,29,72,0.2)]">
                <FiShield className="text-[#E11D48] w-5 h-5" />
                <span className="text-slate-100 font-mono text-sm md:text-base tracking-widest uppercase">{selectedImg.title}</span>
              </div>
            </motion.div>
          </motion.div>
        )}
